...
```{r}
# load the tidyverse
library(tidyverse) #bei packages oft konflikte im zwefel expliziz sein dplyr::filter()
```
...
mean_example <- mean(example_vector)
round(mean_example, 2)
```
...
```
```{r}
example_vector %>% mean() %>% round(2) #tools modify keyboard shortcuts, Alt+.
```
...
vector_with_NAs %>%  
  ...
  head(.,4) %>% 
  mean(.,na.rm = TRUE) %>% 
  as.character(.) #dataargument is hidden mit dots explicit machen
```
...
nerd_red %>% 
  filter(age < 20) %>% 
  select(age)       #hier datarame/tible aber wir wollen einen Vektor wie oben
```
...
```{r}
nerd_red %>% 
  arrange(gender, age) #es sortiert erst gender aufsteigend und innerhalb der stufen das alter
```
...
```{r}
nerd_red %>% 
  mutate(age_character = as.integer(age_character), .before = gender)
```
...
nerd_red %>% 
  mutate(sum_Q1_Q2 = sum(Q1+Q2), .after = nerdy)
```
...
ifelse(some_numbers > 10, "more", "one")
...
case_when(some_more_numbers < 0 ~ some_more_numbers +20,
          some_more_numbers >=0 ~ some_more_numbers)

case_when(some_more_numbers > 0 ~ some_more_numbers -2,
          some_more_numbers <=0 ~ some_more_numbers)

case_when(some_more_numbers == 0 ~ some_more_numbers + 999,
          some_more_numbers =!0 ~ some_more_numbers)

case_when(some_more_numbers < 0 ~ some_more_numbers +20,
          some_more_numbers > 0 ~ some_more_numbers -2,
          some_more_numbers == 0 ~ some_more_numbers + 999)
```
