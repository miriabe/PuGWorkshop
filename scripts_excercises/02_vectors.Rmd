...
```{r}
example_var <- "test" # create dummy variable
example_var2 <- "test2"
```
...
```{r}
rm(example_var)# remove it from the environment
```
...
kuchen
alter <-  c(12, 15, 17, 18, 24)
alter
bmi <- c(14.5, 44.8, 20.7, 22.3, 17.9)
bmi
fem_sex <- c(TRUE, FALSE, TRUE, TRUE, FALSE)
fem_sex
```
...
```{r}
alter_bmi <-  alter + bmi
alter_bmi2 <-  c(alter * bmi)
vector_kombi <- c(my_vector, my_vector2)
reihe <- c(1, 2, 3) + c(4, 5, 6)
#komma kombiniert vektoren
...
1.5) `c(2, 4, 6) - c(1, 0, 1)`
```{r}
punktzahl <- c(2, 4, 6) - c(1, 0, 1)
```


1.6) `c(2, 4, 6) * c(1, 0, 1)`
```{r}
spongebob <- c(2, 4, 6) * c(1, 0, 1)
```


1.7) `c(2, 4, 6) / c(1, 2, 1)`
```{r}
shared_pizza_size <- c(2, 4, 6) / c(1, 2, 1)
```
...
```{r}
c(2, 4, 6, 4, 9, 12) + c(1, 2, 3, 4) #potentially bad yes
```
...
```{r}
head(hotdog_names, 100) #nichts passiert
```
...
```{r}
# use head() in the wrong order, but with argument names; ?head für Info
...
```{r}
# get the third hot dog contestant; with square
hotdog_names[3]
```
...
hotdog_names[max(length(hotdog_names))]
```
...
#hotdog_names[hotdog_2019 == max(hotdog_2019)]
#hotdog_names[max(hotdog_2019) & c(hotdog_genders == "female")]
...
5.4) `table()`
```{r}
table(c(1, 4, NA, 7)) #na wird ignoriert, man kann es aber nicht lösen
```


5.5) `head()` and `tail()`
```{r}
head(c(1, 4, NA, 7)) #egal
```


5.6) `length()`
```{r}
length(c(1, 4, NA, 7)) #macht es einfach richtig
```
...
a_vector[!is.na(a_vector)] #! mean is not
```
